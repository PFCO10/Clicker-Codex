...
function App() {
  ...
  return (
    <div className={`app tier-${tier}`} style={{ '--accent': THEMES[state.theme].accent }}>
      <div className="parallax"></div>
      <header className="topbar glass">
        <div>
          <h1>Neon Idle Forge</h1>
          <p>Theme: {THEMES[state.theme].name}</p>
        </div>
      ...
      <main className="grid">
        <section className="core-panel glass">
          <div className={`core ${shake ? 'shake' : ''}`} onClick={onClick} role="button" tabIndex={0}>
            <div className="core-inner">{THEMES[state.theme].object}</div>
            {floaters.map((f) => (
        ...
        <section className="panel glass">
          ...
          <h2>Abilities</h2>
          {ABILITIES.map((a) => {
            const readyIn = Math.max(0, state.abilityState[a.id].readyAt - Date.now());
            return (
